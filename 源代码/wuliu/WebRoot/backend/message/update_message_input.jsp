...
<div id="formwrapper">
	<h3>编辑资讯信息</h3>
	<form action="MessageServlet?method=update&pageNo=${page.pageNo }&pageSize=${page.pageSize }" method="post">
	<input type="hidden" name="id" value="${message.id }">

	<fieldset>
		<legend>资讯信息信息</legend>
<div>
			<label for="staffIds">资讯信息者</label>
			<select name="staff" id="staffIds" >
				<c:forEach items="${staffList }" var="s" >
				<option value="${s.id }"  <c:if test="${s.id eq message.staff.id }">selected="selected"</c:if>
				>${s.name}</option>
				</c:forEach>
			</select> 
		</div>
		<div>
			...
			<input type="text" name="settime" id="settime" value="${message.settime }" size="8" maxlength="200" /> 
			<br />	
		</div>
		<div>
			<label for="content">资讯信息</label>
			<div style="margin:0px 0px 0px 130px;">
			<textarea rows="10" cols="100" name="content" id="content" >${message.content }</textarea>
			</div>
			<br />	
		</div>
		<div>
			<label for="attachs">添加资讯信息附件</label>
		...
		<div class="enter">
		    ...
		    <input name="return" type="button" class="buttom" value="返回列表页面" onclick="window.location = 'MessageServlet'"/>
		</div>
	</fieldset>
	</form>
</div>
